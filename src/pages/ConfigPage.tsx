...
export default function ConfigPage() {
  ...
  if (loading) return <div className="p-10 text-center font-black text-agribank-maroon animate-pulse uppercase tracking-widest">Đang tải cấu hình...</div>;

  const personal = Array.isArray(configs) ? (configs.find(c => c.key === 'deduct_personal')?.value || 0) : 0;
  const dependent = Array.isArray(configs) ? (configs.find(c => c.key === 'deduct_dependent')?.value || 0) : 0;
