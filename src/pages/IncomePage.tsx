...
import { Search, Filter, User, CreditCard, Users as UsersIcon, ChevronRight, Building2, BarChart3 } from 'lucide-react';
...
export default function IncomePage() {
  ...
  const handleSearch = async () => {
    setLoading(true);
    try {
      const res = await api.get('/api/tax/summary', { params: { month: new Date().getMonth() + 1, year: new Date().getFullYear() } });
  ...
  useEffect(() => {
    if (selectedEmp) {
      api.get(`/api/employees/${selectedEmp.accountNumber}/stats`, { params: { year: new Date().getFullYear() } })
        .then(res => setEmpStats(res.data.map((s: any) => ({ name: `T${s.month}`, value: parseFloat(s.totalIncome) }))))
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col max-h-[700px]">
          <div className="p-6 border-b border-gray-100 bg-gray-50/50">
            <h3 className="font-black text-gray-800 text-xs uppercase tracking-widest">Kết quả tìm kiếm ({Array.isArray(data) ? data.length : 0})</h3>
          </div>
            ...
            <>
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                  <h3 className="font-black text-gray-800 text-xs uppercase tracking-widest">Biểu đồ Thu nhập năm {new Date().getFullYear()}</h3>
                  <BarChart3 size={18} className="text-agribank-maroon" />
                </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={empStats}>
                        ...
                        <Tooltip 
                          ...
                          formatter={(value: any) => [value.toLocaleString('vi-VN') + ' ₫', 'Thu nhập']}
                        />
                        <Bar dataKey="value" fill="#A61D37" radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  ) : (
                    <div className="h-full flex items-center justify-center text-gray-400 font-bold italic text-sm">Chưa có dữ liệu lịch sử năm nay</div>
                  )}
                </div>
              </div>
