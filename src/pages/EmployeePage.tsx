...
import { Search, User, Building2, Users as UsersIcon, Edit2, Check, X, RefreshCw } from 'lucide-react';
...
export default function EmployeePage() {
  ...
  const handleEdit = (emp: any) => {
    ...
    setEditValue(emp.numDependents);
  };

  const handleSave = async (id: string) => {
    try {
      await api.put(`/api/employees/${id}`, { numDependents: editValue });
      toast.success('Cập nhật thành công');
      setEmployees(prev => prev.map(e => e.accountNumber === id ? { ...e, numDependents: editValue } : e));
      setEditingId(null);
    } catch (err) {
      ...
      <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-6 items-center">
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            disabled={loading}
            className="p-4 bg-white border border-gray-100 text-gray-400 hover:text-agribank-maroon hover:border-agribank-maroon/20 rounded-2xl transition-all shadow-sm disabled:opacity-50"
                ...
                <tr key={emp.accountNumber} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="px-8 py-6 text-center">
                    {editingId === emp.accountNumber ? (
                      ...
                      <span className="font-black text-xl text-gray-700">{emp.numDependents}</span>
                    )}
                  </td>
